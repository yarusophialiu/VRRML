...
from PatchDataset_noval2 import PatchDataset # PatchDataset_noval2
from PatchDataset_memmap import PatchDataset_mempmap # PatchDataset_noval2
...
def evaluate(model, val_loader):
    ...
    for mini_batch_idx, (images, metadata) in enumerate(val_loader):
        ...
        images = images.reshape(-1, 3, 64, 64) # (1600, 3, 64, 64) no need to shuffle for validation
...
# structure: train/bistro_path1_seg2_2_480_110_500
if __name__ == "__main__":
    data_train_directory = f'{VRRML}/ML/train_bitratelabel' # ML_smaller
    data_val_directory = f'{VRRML}/ML/validation_bitratelabel' 
    ...
    batch_size = 8 # 450 path folders, 1 path folder for 1 mini batch if batch_size = 1
    ...
    train_image_memmap_file  = f'{VRRML}/ML/train_bitratelabel.dat'
    train_metadata_memmap_file   = f'{VRRML}/ML/train_bitratelabel_metadata_normalize.dat'
    val_image_memmap_file  = f'{VRRML}/ML/validation_bitratelabel.dat'
    val_metadata_memmap_file   = f'{VRRML}/ML/validation_bitratelabel_metadata_normalize.dat'
    train_total_number_of_patches = 288000 # 288000 1200 TODO
    val_total_number_of_patches = 28800 # 28800 120
    image_shape = (64, 64, 3)
    num_patches_per_subfolder = 200
    train_dataset = PatchDataset_mempmap(data_train_directory, train_image_memmap_file, train_metadata_memmap_file, \
                                        train_total_number_of_patches, num_patches_per_subfolder, image_shape, transform=transform)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True)
    val_dataset = PatchDataset_mempmap(data_val_directory, val_image_memmap_file, val_metadata_memmap_file, \
                                        val_total_number_of_patches, num_patches_per_subfolder, image_shape, transform=transform)
