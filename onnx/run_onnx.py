...
onnx_model_path = "onnx_models/vrr_fp32.onnx" # onnx_models vrr_float32_loaded
CONVERT_TO_FLOAT16_MODEL = False # False True   convert model to float16 
if CONVERT_TO_FLOAT16_MODEL:
    model = onnx.load(onnx_model_path)
    model_fp16 = float16.convert_float_to_float16(model)
    onnx.save(model_fp16, "onnx_models/vrr_fp16.onnx")


onnx_model_path_float16 = "onnx_models/smaller_vrr_fp16.onnx" # onnx_models vrr_float32_loaded smaller_vrr_fp16
...
if INFERENCE:
    path = onnx_model_path_float16 if FLOAT16 else onnx_model_path_float32
    print(f'path {path}')
    ort_session = onnxruntime.InferenceSession(path, providers=['CUDAExecutionProvider', 'CPUExecutionProvider']) # onnx_model_path
