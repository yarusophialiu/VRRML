...
# x axis is fps, y axis is refresh rate
if __name__ == "__main__":
    ...
    if PLOT_TEST_RESULT:
        ...
        df = pd.DataFrame({
            'fps': predicted_fps,
            'resolution': predicted_res
        })

        # # Create a pivot table to count occurrences of each (fps, resolution) pair
        # heatmap_data = df.pivot_table(index='resolution', columns='fps', aggfunc='size', fill_value=0)

        # # Plot heatmap
        # plt.figure(figsize=(8, 6))
        # sns.heatmap(heatmap_data, annot=True, cmap='Blues', linewidths=.5, cbar_kws={'label': 'Frequency'}, \
        #             xticklabels=framerate_ticks, yticklabels=resolution_ticks
        #             )
        # Create a pivot table to count occurrences of each (fps, resolution) pair
        heatmap_data = df.pivot_table(index='resolution', columns='fps', aggfunc='size', fill_value=0)

        # Reindex the heatmap data to include all framerate and resolution ticks
        heatmap_data = heatmap_data.reindex(index=resolution_ticks, columns=framerate_ticks, fill_value=0)

        # Plot heatmap with custom x and y axis ticks
        plt.figure(figsize=(8, 6))
        sns.heatmap(heatmap_data, annot=True, cmap='Blues', linewidths=.5, cbar_kws={'label': 'Frequency'},
                    xticklabels=framerate_ticks, yticklabels=resolution_ticks)


        plt.title('Heatmap of Refresh Rate vs Resolution')
        plt.xlabel('Refresh Rate (FPS)')
        plt.ylabel('Resolution (px)')
        # plt.xticks(framerate_ticks)
